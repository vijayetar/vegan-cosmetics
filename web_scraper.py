...
# TO DO 
# Redundant code [x]
# Everything we generate is being lost, we have to save it somehow, so, Ahmed suggested making a class with item name, price, ingredients, and the webiste that we got it from. These things need to be available. THEN we can save it to a txt file or db
# 
...
#########################  Dictionary ################################
web_sites = [
    {'website': '100 Percent Pure','url':"https://www.100percentpure.com/collections/fruit-dyed-makeup/products/2nd-skin-corrector", 
    ...
    {'website': 'Mac Cosmetics','url':"https://www.maccosmetics.com/product/13854/310/products/makeup/lips/lipstick/matte-lipstick#!/shade/Chili", 
    'cost': ['div','product-price-v1 js-product-price-v1'], 
    'results': ['p', 'js-product-full-iln-listing'],
    'name': ['h1','product-full__name']}, 
    {'website': 'Thrive Causemetics','url':"https://thrivecausemetics.com/products/liquid-light-therapy-face-serum", 
    'cost': ['span','product-main--price-item product-main--price-normal'], 
...
def get_contents():
  ...
  results = soup.findAll(ele_results, class_ = ele_results_class)
  name = soup.findAll(ele_name, class_ = ele_name_class)
  print("*" * 50)
  print(website)
...
def url_contents():
    for i in range(len(web_sites)):
        ...
        print(i)   
        get_contents(website, url, ele_cost, ele_cost_id, ele_results, ele_results_class, ele_name, ele_name_class)
